...
## dag_preprocess.py
The `dag_preprocess.py` module provides functions to compute the longest path length `lp` and the stratified decomposition `G'` for a given unweighted DAG `G` without multiple edges. These methods are described in Section 5.1 of the paper.
...
```python
lp, betti, basis = max_path_homology(G, calculate_basis, report_sparsity)
```
Here:
...
- If `report_sparsity == True`, then sparse-matrix diagnostics are
  printed during the recursion. For each processed weakly connected
  component, the function reports the initial cycle-space matrix `V`
  at layer 0 and, at each subsequent layer, the assembled matrix `A`
  and the updated cycle-space matrix `V`, whenever they are
  constructed. Each line contains the component and layer indices,
  matrix shape, number of nonzero entries (`nnz`), sparse density, and
  CSR storage in MiB, where the sparse density is computed by
  $$
  \text{density} = \dfrac{\text{nnz}}{\text{total number of elements}}.
  $$
  The reported storage includes only the CSR `data` and `indices`; it is not the total memory usage of the algorithm.
...
`experiment_functions.py`, `stratified_gamma1_to_3.py` and `stratified_gamma4_to_10.py`: Implement additional functions and simulations discussed in Section 6.1 of the paper.
